...
def run_cache_builder():
    ...
    if not candidate_logs:
        print("-> [결과] 정제할 신규 로그(satisfaction= 1)가 없습니다. 작업을 종료합니다.")
        conn.close()
        return
    ...
    if final_docs_to_cache:
        try:
            print(f"-> [저장] 총 {len(final_docs_to_cache)} 개의 정제된 문서를 '{CACHE_DB_PATH}'에 저장합니다.")
    ...
    if processed_ids:
        try:
            print(f"-> [라벨링] 처리 완료된 {len(processed_ids)} 개의 로그를 processed_for_cache= 1로 업데이트합니다.")
    ...
    conn.close()
    end_time = time.time()
    print(f"--- [Cache Builder] 작업 완료 (총 소요 시간: {end_time - start_time:.2f} 초) ---")
